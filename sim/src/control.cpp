...
public:
    controlLayer()
    {
        ...
        sub_odom = nh.subscribe("vision_odom",1000,&controlLayer::onmsg_odom,this);
...
double stanley_method(double x,double y,double r,double dis)
{
        //注意，turtlebot中r为-1，1，朝向以x轴朝向为0，逆时针正，顺时针负
        double k = 2;
        double v = 0.2;
...
void onmsg_odom(const nav_msgs::Odometry& msg)
{
    ...
    // {
        double delta_sum = stanley_method(x,y,r,dis);
        move_cmd.linear.x = 0.2;
