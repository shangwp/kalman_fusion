...
public:
    task()
    {
        pub_target = nh.advertise<geometry_msgs::Vector3>("target",1000);
        ...
        for(int i=0;i<total_num;i++)
        {
            ...
            target_point[i*3] = 0.5*cos(theta-M_PI_2) + 3;
            target_point[i*3+1] = 0.5*sin(theta-M_PI_2) + 2.5;
            ...
            target_log<<setprecision(12)<<target_point[i*3]<<","<<setprecision(12)<<target_point[i*3+1]<<",";
            target_log<<setprecision(12)<<target_point[i*3+2]<<endl;
        }
        uwb_num = 0;
        camera_num =0;
    }
