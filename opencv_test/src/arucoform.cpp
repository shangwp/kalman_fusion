...
//111
Mat R_CAMERA = (Mat_<double>(3, 3) <<-0.996961581511579, -0.01108759919077975, 0.07710168697321589,
 -0.01198209621334884, 0.9998660601314904, -0.01114859486512579,
  -0.07696774883199585, -0.01203856059989225, -0.9969609012887203);
Mat T_CAMERA = (Mat_<double>(3, 1) <<2.357312239155356,
 -2.290004681865794,
 3.324415225716706);
...
void init_form()
{
	...
	pub_vision = nh.advertise<nav_msgs::Odometry>("vision",1000);
}
int ArucoCreat()
{
	int markersX = 1;
...
void calculateRealPoint(vector<Point2f> camera2d, Mat R, Mat t)
{
	...
	vision.pose.pose.position.x =  wcPoint.at<double>(0, 0) ;
    vision.pose.pose.position.y =  wcPoint.at<double>(1, 0);
	cout<<"I'm here."<<endl;
	pub_vision.publish(vision);
	/*
